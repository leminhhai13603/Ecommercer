...
import { getPersonalizedRecommendations } from '../api';
...
const PersonalizedRecommendations = () => {
    ...
    useEffect(() => {
        const fetchRecommendations = async () => {
            try {
                ...
                setError('Vui lòng đăng nhập để xem đề xuất cá nhân');
            } finally {
                setLoading(false);
            }
        };
    ...
    const nextSlide = () => {
        ...
        if (currentIndex + 4 >= recommendations.length) {
    ...
    const prevSlide = () => {
        ...
        if (currentIndex === 0) {
            ...
            const lastIndex = Math.floor((recommendations.length - 1) / 4) * 4;
    ...
    if (recommendations.length === 0) {
        return (
            <div className="alert alert-info text-center my-4" role="alert">
                Chưa có đề xuất nào. Hãy tương tác với các sản phẩm để nhận đề xuất phù hợp!
            </div>
        );
    }

    return (
        <div className="personalized-recommendations my-5">
            <div className="section-header mb-4">
                ...
                <p className="text-center text-muted">
                    Dựa trên lịch sử xem và mua sắm của bạn
                </p>
            </div>
